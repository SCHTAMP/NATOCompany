		...
		if ((name player find "RC" !=-1) || (name player find "JP" !=-1) || (name player find "JK" !=-1) || (name player find "JM" !=-1) || (name player find "Zewa" !=-1)) then {
			if ((alive _veh) && (player distance _veh < 10) && (_veh != vehicle player)) then {
				...
				sleep 3;
				hintSilent "";
				};//Шанс 25%, что попадёшь в боекомплект и техника взорвётся
				
				if ((_vrandom >=27) && (_vrandom <=70)) then {
				hint format ["Попытка %1 устарнить экипаж %2 неудачна.", name player, _veh];
				sleep 3;
				hintSilent "";
				};//Шанс 25%, что не произойдёт ничего, просто неудачная попытка
