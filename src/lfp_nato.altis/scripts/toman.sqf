h = [] spawn {
	...
	_ctrlEdit ctrlAddEventHandler ["KeyUp", {params ["_ctrl", "_key"];
		...
		if (_isValid) then {
			...
			if (_number isEqualTo 186258) 
			then {player setposatl [getpos b0 select 0, getpos b0 select 1, getposatl b0 select 2]; titleText ["Вас провожают до этой комнаты", "BLACK FADED", 0.2];} 
